...
export default function ExamenInstrucciones() {
  ...
  return (
    <div style={s.page}>
      ...
      {/* Header — 0ms delay */}
      <header style={{ ...s.header, animation: 'fadeSlideDown 260ms cubic-bezier(0.23,1,0.32,1) both' }}>
      ...
      <div style={s.container}>
        {/* Saludo — 60ms delay */}
        <div style={{ ...s.card, animation: 'fadeSlideUp 260ms cubic-bezier(0.23,1,0.32,1) 60ms both' }}>
        ...
        {/* Instrucciones — cards con stagger 80ms cada una */}
        <div style={{ ...s.card, animation: 'fadeSlideUp 260ms cubic-bezier(0.23,1,0.32,1) 60ms both' }}>
          <div style={s.sectionTitle}><i className="ti ti-info-circle" style={{ fontSize: 16, color: '#7c3aed' }} /> Instrucciones del examen</div>
          <div style={s.grid}>
            {INSTRUCCIONES.map(({ icon, color, title, desc }, idx) => (
              <div key={title} style={{ ...s.instruccionItem, animation: `fadeSlideUp 240ms cubic-bezier(0.23,1,0.32,1) ${idx * 80}ms both` }}>
        ...
        {/* Warning — 360ms delay */}
        <div style={{ ...s.warningCard, animation: 'fadeSlideUp 240ms cubic-bezier(0.23,1,0.32,1) 360ms both' }}>
        ...
        {/* Confirmación + botón */}
        <div style={{ ...s.card, animation: 'fadeSlideUp 240ms cubic-bezier(0.23,1,0.32,1) 420ms both' }}>
          <label style={s.aceptaLabel}>
            <input
              type="checkbox" checked={acepta}
              onChange={e => setAcepta(e.target.checked)}
              style={{ width: 17, height: 17, accentColor: '#7c3aed', cursor: 'pointer', flexShrink: 0, marginTop: 1 }}
            />
            <span>He leído las instrucciones y entiendo las condiciones del examen. Estoy listo para comenzar.</span>
          </label>

          {/* Botón aparece solo cuando acepta, con transición opacity + transform */}
          <div style={{
            marginTop: 16,
            opacity: acepta ? 1 : 0,
            transform: acepta ? 'translateY(0) scale(1)' : 'translateY(8px) scale(0.96)',
            transition: 'opacity 200ms cubic-bezier(0.23,1,0.32,1), transform 200ms cubic-bezier(0.23,1,0.32,1)',
            pointerEvents: acepta ? 'auto' : 'none',
          }}>
            <button
              onClick={iniciar}
              disabled={!acepta || iniciando}
              style={{
                ...s.btn,
                cursor: (!acepta || iniciando) ? 'not-allowed' : 'pointer',
                opacity: iniciando ? 0.75 : 1,
                transform: btnHover && acepta && !iniciando ? 'translateY(-1px)' : 'translateY(0)',
                boxShadow: btnHover && acepta && !iniciando ? '0 10px 28px rgba(124,58,237,0.35)' : '0 6px 20px rgba(124,58,237,0.25)',
              }}
